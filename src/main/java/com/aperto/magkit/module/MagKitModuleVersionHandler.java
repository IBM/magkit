...
public class MagKitModuleVersionHandler extends DefaultModuleVersionHandler {
    ...
    private final Task _addValidatorFilterTask = new ArrayDelegateTask("Filter", "Add the Validator filter.", new Task[]{
        ...
        new SetPropertyTask(ContentRepository.CONFIG, "/server/filters/validator", "class", "com.aperto.nektar.filter.HtmlValidatorFilter"),
    ...
    public MagKitModuleVersionHandler() {
        DeltaBuilder builder = DeltaBuilder.update("1.0.0", "Upgrading to 1.0.0 / Magnolia 3.5");
        builder.addTask(_addCmsFilterBypassTask)
            .addTask(_addValidatorFilterTask)
            .addTask(_addValidatorFilterBypassTask);
        register(builder);
    }
