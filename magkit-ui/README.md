...
- Authoring Field Enhancements:
  ...
  - Specific multi-value field support (`SpecificMultiFieldDefinition`, `SpecificMultiValueFieldDefinition`, `SpecificMultiFormView`) for structured list input.
...
### 7. Recommended Multi-Value Specific Field
```yaml
fields:
  keywords:
    class: de.ibmix.magkit.ui.dialogs.fields.SpecificMultiValueFieldDefinition
    name: keywords
    label: Keywords
    itemProviderClass: com.example.KeywordsProvider
```

### 8. Translate UI Label Programmatically
```java
...
### 9. Auto-Approve Link Normalization
```java
